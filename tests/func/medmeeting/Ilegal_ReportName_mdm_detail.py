...
# pass number
pass_num = 0
file = open("Ilegal_ReportName_mdm_detail.txt", "w")
#Scenario:
...
with open('Ilegal_ReportName_mdm_detail.csv', 'w',newline='') as csv_file:
...
while True:
    ...
    try:
        ...
        if bool(re.search(en_reg, row[5])) or row[5] is "":
            contain_report = True
        ...
        if contain_report:
            ...
            fail_num += 1
            with open('Ilegal_ReportName_mdm_detail.csv', 'a+', newline='') as csv_file:
                csv_writer = csv.writer(csv_file)
