...
n = 1
# Case rsult storage
file = open("Ilegal_Event_Name_man_event.txt", "w")
####################################################################
#Scenario:
...
with open('Ilegal_Event_Name_man_event.csv', 'w', newline='') as csv_file:
...
while True:
    ...
    try:
        contain_Event_Name=tools.en_cn_length_check(row[7],1,4)
        if contain_Event_Name:
            pass_num += 1
        else:
            fail_num += 1
            with open('Ilegal_Event_Name_man_event.csv', 'a+', newline='') as csv_file:
                csv_writer = csv.writer(csv_file)
