...
n = 1
# Case rsult storage
file = open("Ilegal_Origin_Name_man_event.txt", "w")
####################################################################
#Scenario:
...
with open('Ilegal_Name_man_event.csv', 'w', newline='') as csv_file:
...
while True:
    ...
    try:
        ...
        if bool(re.search(en_reg, row[5])):
            ...
            contain_name = True
            if (len(row[5]) < 2) or len(row[5]) > 4:
                contain_name = False
        if contain_name:
            pass_num += 1
        else:
            fail_num += 1
            with open('Ilegal_Name_man_event.csv', 'a+', newline='') as csv_file:
                csv_writer = csv.writer(csv_file)
