...
n = 1
# Case rsult storage
file = open("Ilegal_Name_man_ass.txt", "w")
####################################################################
#Scenario:
...
with open('Ilegal_Name_man_ass.csv', 'w', newline='') as csv_file:
...
while True:
    ...
    try:
        contain_Name = name_check.en_cn_length_check(row[4],2,4)
        if contain_Name:
            pass_num += 1
        else:
            fail_num += 1
            with open('Ilegal_Name_ass.csv', 'a+', newline='') as csv_file:
                csv_writer = csv.writer(csv_file)
