...
n = 1
# Case rsult storage
file = open("Ilegal_Name_man_guide.txt", "w")
####################################################################
#Scenario:
...
with open('Ilegal_Name_man_guide.csv', 'w', newline='') as csv_file:
...
while True:
    ...
    try:
        ...
        if bool(re.search(en_reg, row[1])):
            ...
            contain_name = True
            if (len(row[1]) < 2) or (len(row[1] > 4)):
                contain_name = False
        if contain_name:
            pass_num += 1
        else:
            fail_num += 1
            with open('Ilegal_Name_man_guide.csv', 'a+', newline='') as csv_file:
                csv_writer = csv.writer(csv_file)
